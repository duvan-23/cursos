...
if(cluster.isPrimary && modo =="CLUSTER" || modoCluster && cluster.isPrimary){
    ...
    io.on('connection', async (socket) => {
        ...
        socket.emit('products', await contenedorMongo.listarProductos())

        socket.on('new-product', async data => {
            // productos.push(data)
            await contenedorMongo.insertarProductos(data);
            io.sockets.emit('products',  await contenedorMongo.listarProductos())
        })
    })
